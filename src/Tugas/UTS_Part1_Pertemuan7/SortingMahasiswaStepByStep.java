        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int j = 0; j < n - i - 1; j++) {
                if (arr[j].nilai < arr[j + 1].nilai) { // descending
                    ...
                    swapped = true;
                    step++;
                    printStep(arr, "BUBBLE SORT - setelah swap", step);
                }
            }
            if (!swapped) break;
        }
    ...
    public static void insertionSortStep(Mahasiswa[] arr) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            while (j >= 0 && arr[j].nilai < key.nilai) {
                ...
                j = j - 1;
                step++;
                printStep(arr, "INSERTION SORT - setelah pergeseran", step);
            }
            arr[j + 1] = key;
            step++;
            printStep(arr, "INSERTION SORT - setelah menempatkan key", step);
        }
    }

    // Selection Sort dengan step
    public static void selectionSortStep(Mahasiswa[] arr) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (maxIdx != i) {
                ...
                arr[i] = temp;
                step++;
                printStep(arr, "SELECTION SORT - setelah swap", step);
            }
        }
    }
